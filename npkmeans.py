...
def distances(X, Y):
    ...
    Part of a non-scipy kmeans implementation that can run on pypy.
    """
...
# if also modified in a number of places
def cluster_centroids(data, clusters, k=None):
    ...
    for i in range(k):
        # The original code was a little abstruse, AND didn't appear
...
def npkmeans(data, k=None, centroids=None, steps=20):
    ...
    """
    # If after 20 reinitializations, you STILL think there's only one cluster--
    # fine.
    all_clusters = []
    all_ssd = []
    for tries in range(20):
        ...
        if np.all(clusters[0]==clusters):
            ...
            # Since centroids have already been assigned by this point, if you
            # just start at the top of the loop again, you don't reinitialize,
            # which is necessary to get out of whatever point you're stuck in
            # that resulted in everybody being in the same cluster.
            centroids = data[np.random.choice(np.arange(len(data)), k, False)]
        #else:
        #    return all_clusters
    return all_clusters[all_ssd.index(min(all_ssd))]
